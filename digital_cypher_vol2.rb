...
def decode(code, key)
  keys = key.to_s.chars
  keys = keys * (code.size / keys.size) + keys[0, (code.size % keys.size)]
  keys.map!(&:to_i)
  code.zip(keys).map { |pair| pair.reduce(:-) }.map { |nb| (96 + nb).chr }.join
end

# other solution

def decode(code, key)
  code.zip(key.to_s.chars * code.size).map { |ord, n| (ord + 96 - n.to_i).chr }.join
end

# ne pas oublier que la taill retournée par zip dépend de la taille du 1er array
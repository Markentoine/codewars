...
def growing_plant(upspeed, downspeed, threshold)
  days = 1
  height = 0
  loop do
    height += upspeed
    break if height >= threshold
    height -= downspeed
    days +=1
  end
  days
...
p growing_plant(5,2,0)      == 1
p growing_plant(5,2,5)      == 1
p growing_plant(5,2,6)      == 2

# others ----------------------------------------------------------------------

def growing_plant(up, down, height, days = 1)
  (height - up).positive? ? growing_plant(up, down, height - up + down, days + 1) : days
end
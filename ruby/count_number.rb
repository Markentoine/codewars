...
le prombleme ici c'est le Big(O) du programme qui a vite fait d'éclater la mémoire
=end


def count_number(n, x)
	multiples = *(1..n).select { |num| x % num == 0 }
	multiples.product(multiples).reduce(0) { |count, sub| sub[0] * sub[1] == x ? count += 1 : count }
end 

p count_number(6, 12)
p count_number(100000,1000000000)

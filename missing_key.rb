...
Example
 ...
 input : two arrays
 output : a string representing the key ordered
 
 algo !
 iterate trough arrays an associated each letter with the other; iterate trough the result and delete if similar similar letters, sort the result and join it to be returned
=end
 
def find_the_key(messages, secrets)
 str_message = messages.join.gsub(' ', '').chars
 str_secrets = secrets.join.gsub(' ', '').chars
 str_message.zip(str_secrets).map(&:sort).uniq.sort_by { |pair| pair[0] }.map { |pair| pair[0] == pair[1] ? nil : pair }.compact.flatten.join
end

p find_the_key([ "dance on the table", "hide my beers", "scouts rocks" ], [ "egncd pn thd tgbud" ,"hked mr bddys" ,"scplts ypcis" ])
...
The function returns the "best" sum : 

ie the biggest possible sum of k distances less than or equal to the given limit t, if that sum exists,
or otherwise nil, null, None, Nothing, depending on the language. With C++, C, Rust, Swift return -1.

Examples:

ts = [50, 55, 56, 57, 58] choose_best_sum(163, 3, ts) -> 163

xs = [50] choose_best_sum(163, 3, xs) -> nil (or null or ... or -1 (C++, C, Rust, Swift)
...
def choose_best_sum(t, k, ls)
  ...
  comb = ls.combination(k).to_a
  distances = comb.map { |sub| sub.reduce(:+) }
  return nil if distances.none? { |d| d <= t }
  to_goal_distances = distances.map { |s| t - s }
  min_diff = to_goal_distances.reduce(to_goal_distances.max) { |max, n| n < max && n >= 0 ? n : max }
  index = to_goal_distances.index(min_diff)
  comb[index].reduce(:+)
end
...
def choose_best_sum(t, k, ls)
  ls.combination(k).collect { |ds| ds.inject(:+) }.reject { |d| d > t }.max # penser + à #reject!!
end